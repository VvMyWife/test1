...
_LAST_PADDLE_RUNTIME_INFO: JsonDict = {}
...
def paddle_runtime_info() -> JsonDict:
    with _PADDLE_RUNTIME_LOCK:
        return dict(_LAST_PADDLE_RUNTIME_INFO)
...
def _prepare_paddle_runtime() -> None:
    ...
    _update_paddle_runtime_info(
        {
            "python_executable": sys.executable,
            "python_path_head": sys.path[:3],
            "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
            "mkldnn_enabled": os.environ.get("FLAGS_use_mkldnn"),
        }
    )


def _resolve_paddle_device(options: Mapping[str, Any]) -> str | None:
    explicit_option = _coerce_optional_str(options.get("paddle_device"))
    if explicit_option:
        _record_device_resolution(
            device=explicit_option,
            source="option",
            details={"reason": "explicit paddle_device option"},
        )
        return explicit_option

    configured_env = _coerce_optional_str(os.environ.get("MINERU_PADDLE_DEVICE"))
    if configured_env:
        _record_device_resolution(
            device=configured_env,
            source="env",
            details={"reason": "MINERU_PADDLE_DEVICE"},
        )
    ...
    try:
        ...
        _record_device_resolution(
            device=None,
            source="fallback",
            details={"reason": "paddle import failed", "error": repr(exc)},
        )
        return None

    compiled_with_cuda = bool(paddle.is_compiled_with_cuda())
    try:
        cuda_device_count = int(paddle.device.cuda.device_count()) if compiled_with_cuda else 0
    except Exception as exc:
        _record_device_resolution(
            device=None,
            source="fallback",
            details={
                "reason": "failed to query paddle cuda device count",
    ...
    if not compiled_with_cuda or cuda_device_count <= 0:
        _record_device_resolution(
            device=None,
            source="fallback",
            details={
                "reason": "no cuda-capable paddle runtime available",
                "compiled_with_cuda": compiled_with_cuda,
    ...
    device, auto_details = _auto_select_paddle_gpu()
    auto_details["compiled_with_cuda"] = compiled_with_cuda
    auto_details["cuda_device_count"] = cuda_device_count
    auto_details["paddle_version"] = getattr(paddle, "__version__", None)
    if device:
        _record_device_resolution(device=device, source="auto", details=auto_details)
        return device

    _record_device_resolution(device=None, source="fallback", details=auto_details)
    return None
...
def _update_paddle_runtime_info(payload: Mapping[str, Any]) -> None:
    with _PADDLE_RUNTIME_LOCK:
        _LAST_PADDLE_RUNTIME_INFO.update(_jsonable(dict(payload)))


def _record_device_resolution(
    *,
    device: str | None,
    source: str,
    ...
    runtime_payload = {
        ...
        "resolution_details": dict(details),
    }
    _update_paddle_runtime_info(runtime_payload)
    LOGGER.info("Resolved Paddle device: %s source=%s details=%s", device, source, dict(details))


def _auto_select_paddle_gpu() -> tuple[str | None, JsonDict]:
    records = _parse_nvidia_smi_gpu_query_output(_run_nvidia_smi_gpu_query())
    if not records:
        return None, {"reason": "nvidia-smi unavailable or returned no GPU records"}

    process_counts = _parse_nvidia_smi_process_query_output(_run_nvidia_smi_process_query())
    visible_records = _filter_visible_gpu_records(records, os.environ.get("CUDA_VISIBLE_DEVICES"))
    if not visible_records:
        return None, {
            "reason": "no visible GPUs matched CUDA_VISIBLE_DEVICES",
            "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
        }

    enriched_records = [
        _GpuRuntimeRecord(
            host_index=record.host_index,
            uuid=record.uuid,
            memory_total_mib=record.memory_total_mib,
            memory_used_mib=record.memory_used_mib,
            process_count=process_counts.get(record.uuid, 0),
    ...
    candidates = sorted(
        enriched_records,
        key=lambda item: (-item.free_memory_mib, item.process_count, item.host_index),
    )
    selected = candidates[0]
    details: JsonDict = {
        "reason": "selected visible GPU with most free memory",
        "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
        "candidates": [
            {
                "host_index": item.host_index,
                "local_index": item.local_index,
                "uuid": item.uuid,
                "memory_total_mib": item.memory_total_mib,
                "memory_used_mib": item.memory_used_mib,
                "free_memory_mib": item.free_memory_mib,
                "process_count": item.process_count,
            }
            for item in candidates
        ],
    }
    return f"gpu:{selected.local_index}", details


def _run_nvidia_smi_gpu_query() -> str | None:
    try:
...
def _run_nvidia_smi_process_query() -> str | None:
    try:
        completed = subprocess.run(
            [
                "nvidia-smi",
...
def _parse_nvidia_smi_gpu_query_output(output: str | None) -> list[_GpuRuntimeRecord]:
    if output is None:
        return []
    records: list[_GpuRuntimeRecord] = []
...
def _parse_nvidia_smi_process_query_output(output: str | None) -> dict[str, int]:
    if output is None:
        return {}
    counts: dict[str, int] = {}
    for raw_line in output.splitlines():
...
def _filter_visible_gpu_records(
    records: Sequence[_GpuRuntimeRecord],
    cuda_visible_devices: str | None,
) -> list[_GpuRuntimeRecord]:
    if cuda_visible_devices is None or not cuda_visible_devices.strip():
        return [
            ...
            for index, record in enumerate(sorted(records, key=lambda item: item.host_index))
        ]

    ordered_visible: list[_GpuRuntimeRecord] = []
    token_map = {str(record.host_index): record for record in records}
    uuid_map = {record.uuid.lower(): record for record in records if record.uuid}
    for local_index, token in enumerate(item.strip() for item in cuda_visible_devices.split(",") if item.strip()):
        record = token_map.get(token) or uuid_map.get(token.lower())
        if record is None:
            continue
        ordered_visible.append(
            _GpuRuntimeRecord(
                host_index=record.host_index,
                uuid=record.uuid,
                memory_total_mib=record.memory_total_mib,
